...
const sql = "REPLACE INTO `edudata`.`summer_21` (`date`, `Player`, `Role`, `KDA`, `CSM`, `GPM`, `Vision Score`, `DPM`, `KP`, `GD15`, `Result`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
var connection;
...
const CrawlingMatchResult = async () => {
  const res = await axios.get(`https://gol.gg/tournament/tournament-matchlist/LCK%20Summer%202021/`);
  const $ = cheerio.load(res.data);
